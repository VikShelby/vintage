    ...
    >
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <Link href="/" className="font-serif text-2xl flex items-center gap-1 font-bold text-espresso">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {["Home", "Shop", "Collections", "New Arrivals", "About"].map((item) => (
            ...
            >
              <form onSubmit={handleSearch} className="relative max-w-md mx-auto">
                <motion.input
                  ...
                  className="w-full pl-4 pr-12 py-3 border border-warm-brown/20 rounded-sm bg-ivory text-espresso placeholder-espresso/60 focus:outline-none focus:ring-2 focus:ring-rust/20 focus:border-rust/30"
                  autoFocus
                />
                <motion.button
                  type="submit"
                  ...
                  className="absolute right-2 top-[10px] transform -translate-y-1/2 p-2 text-espresso hover:text-rust transition-colors"
              ...
              <motion.p
                ...
                className="text-center text-sm text-espresso/60 mt-2"
              >
          ...
          {isMenuOpen && (
            ...
            >
              <nav className="flex flex-col space-y-4">
                {["Home", "Shop", "Collections", "New Arrivals", "About"].map((item, index) => (
                  <motion.div
                    key={item}
                ...
                >
                  <form onSubmit={handleSearch} className="relative">
                    <input
                      ...
                      onChange={(e) => setSearchTerm(e.target.value)}
                      placeholder="Search vintage treasures..."
                      className="w-full pl-4 pr-12 py-2 border border-warm-brown/20 rounded-sm bg-ivory text-espresso placeholder-espresso/60 focus:outline-none focus:ring-2 focus:ring-rust/20 focus:border-rust/30"
                    />
                    <button
                      type="submit"
                      className="absolute right-2 top-1/2 transform -translate-y-1/2 p-1 text-espresso hover:text-rust transition-colors"
                      aria-label="Submit search"
                    >
                      <Search className="w-4 h-4" />
                    </button>
